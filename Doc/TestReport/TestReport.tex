...
To test this functional requirement, we can use a combination of code inspection and manual testing. We have to verify that once an asteroid collides with a player, both the asteroid and the player are destroyed. We can verfiy this with code inspection, since the asteroid is a circle, it is easy to calculate if the ship collides with the asteroid, by simply checking the distance between the center points of the two objects. If the distance between them is less than the radius of both obejcts combined, we know they collide. This property can be verified with code inspection, as it has this logic, as well as with manual testing, which has shown this property to be consistent and working.
\subsection{Asteroid separation}
To test this functional requirement we can use a combination of code inspection and manual testing. We have to verify that when an asteroid (that is not the smallest size) is destroyed it splits into 3 smaller asteroids with random speed and direction. Smaller asteroids can also be destroyed and will split up if they are not the smallest size. We can verfiy these properties with a code inspection, as we can see in the code that each asteroid that is destroyed creates children only if it not the smallest size, as well as showing that they split into 3 random asteroids. This has also been verified with manual testing.

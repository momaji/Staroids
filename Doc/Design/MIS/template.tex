...
Bullet():
\begin{itemize}
    \item transition: $ timeOut, vel, x, y, r, velx, vely = \mbox{200, {}, Player.x + 4/3 * Player.r * cos(Bullet.a), Player.Y - 4/3 * Player.r * sin(Bullet.a), 1, Player.Vel.x + BULLET_EXTRA * cos(Bullet.a), Player.Vel.y + BULLET_EXTRA * -sin(Bullet.a) }$
...
Alien():
\begin{itemize}
    \item transition: $ timeSpawn, timeOut, xOrY, lOrR, acc, r = \mbox{ALIEN_SPAWN, 50, true, true, (0,0), 12.5 }$
...
Asteroid():
\begin{itemize}
    \item transition: $ x, y, scale, r, children, vel, velx, vely = \mbox{ round(random(0,1)*CVS_WIDTH), round(random(0,1)*CVS_HEIGHT), scale, 5*scale, [], {}, +/-random(0,1)*3, +/-random(0,1)*3 }$
    \item output: $out := Asteroid$
    \item exception: None
\end{itemize}

%local function cvs width

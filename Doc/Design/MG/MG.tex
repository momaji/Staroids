...
\item[\refstepcounter{acnum} \actheacnum \label{acUtilites}:] Enlarging of the playing screen and the scale of all on screen objects.
...
\begin{description}
\item[\refstepcounter{ucnum} \uctheucnum \label{ucUtilities}:] The control mapping for the player ship.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucUtilities}:] Support for two players to be active at the same time.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucGameobject}:] Player object support for a cooperative or adversarial game mode.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucGameObject}:] The base game
  object class that the player asteroid and bullets are based on is not expected
  to be changed.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucSoundModule}:] The sound module
  itself is not likely to be changed as all the sounds for game actions have already
  been defined in it. If a sound is to be changed it doesn't require the module to
  updated, just the file to be specifically named to replace the old sound.
\end{description}
...
\item [\refstepcounter{mnum} \mthemnum \label{mBHa}:] appendHead Module
...
\item [\refstepcounter{mnum} \mthemnum \label{mSDs}:] Sound Module
\end{description}
...
\subsubsection{Sound Module (\mref{mSDs})}
\begin{description}
...
\subsubsection{Utilites Module (\mref{mBHu})}
\begin{description}
\item[Secrets:]The values of all constants for the game such as max speed ship size ect, and variables to handle user inputs.
\item[Services:] Funtions to test for if a key is pressed. The game object consisting of score, lives, asteroids ect. As well as functions to
...
\acref{acAppend} & \mref{mBHa}\\
\acref{acGamestate} & \mref{mSDgs}\\
\acref{acGameobject} & \mref{mSDgo}\\
\acref{acUtilities} & \mref{mBHu}\\
\acref{acSound} & \mref{mSDs}\\
%\acref{acHardware} & \mref{mHH}\\
%\acref{acInput} & \mref{mBHa}\\
%\acref{acParams} & \mref{mSDgs}\\
%\acref{acVerify} & \mref{mSDgo}\\
%\acref{acOutput} & \mref{mBHu}\\
%\acref{acVerifyOut} & \mref{mSDs}\\

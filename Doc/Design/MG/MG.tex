...
Nov 7/18 & 0.25 & Added unlikely changes\\
\bottomrule
\end{tabularx}
\end{table}
...
\item[\refstepcounter{ucnum} \uctheucnum \label{acUtilities}:] Support for two players to be active at the same time.
\item[\refstepcounter{ucnum} \uctheucnum \label{acGameobject}:] Player object support for a cooperative or adversarial game mode.
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucGamestate}:] The state structure of the core game and how the states transfer into one another.
